...
信息获取通道包括：
    ...
    * https://xueqiu.com/S/SZ002353/FHPS
...
__all__ = ["async_get_fhsp", "get_fhsp"]
...
async def async_get_fhsp(session, symbol, data_path=None, return_df=True):
    """
    从雪球获取分红送配股数据
    ...
    logger.info("start download fhsp from xueqiu for %s...", symbol)
    ...
    async with session.get(url, params=params) as resp:
        if resp.status != 200:
            logger.warning("get fhsp from %s failed: %s", url, resp.status)
            return None
        data = await resp.json()
        if "list" not in data:
            logger.warn("no fhsp data downloaded for %s from %s: % ", symbol, url, data)
        fhsp_data = data["list"]

    if not fhsp_data:
        logger.warn("no fhsp data downloaded for %s from %s, return None", symbol, url)
        return None

    logger.info("download fhsp for %s from %s finish", symbol, url)
    if data_path:
        data_path = Path(data_path) / MODULE_DATA_DIR / "fhsp"
        os.makedirs(data_path, exist_ok=True)
        data_fname = "-".join((symbol, date_str)) + ".json"
        data_file = data_path / data_fname
        with data_file.open("w", encoding="utf-8") as dataf:
            json.dump(fhsp_data, dataf, indent=4, ensure_ascii=False)

    if not return_df:
        return fhsp_data

    df = pd.DataFrame(fhsp_data)
    # set index
    # df.set_index("bonusimpdate", inplace=True)
    return df


@add_doc(async_get_fhsp.__doc__)
def get_fhsp(*args, **kwargs):
    ret = fetch_http_data(async_get_fhsp, *args, **kwargs)
...
@stock_cli_group.command("fhsp")
@click.pass_context
def fhsp_cli(ctx, symbol, save_path, show):
    """从雪球获取分红送配数据"""
    if not save_path:
        save_path = ctx.obj["data_path"]
    data = get_fhsp(symbol, save_path)
    if show:
        click.echo(data)
